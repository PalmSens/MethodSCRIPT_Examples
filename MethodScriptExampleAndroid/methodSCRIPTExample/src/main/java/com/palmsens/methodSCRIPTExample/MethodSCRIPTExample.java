...
public class MethodSCRIPTExample extends AppCompatActivity {
    ...
    private boolean writeToDevice(String script) {
        ...
        if (ftDevice != null) {
            synchronized (ftDevice) {
                ...
                if (bytesWritten == writeByte.length) {                                                 // Verify if the bytes written equals the total number of bytes sent
                    isWriteSuccess = true;
                }
            }
        }
    ...
    private void processResponse(String line){
        if('e' == line.charAt(0))                        // Remove the beginning character of the response if it is 'e'
            line = line.substring(1);
        if (mIsVersionCmdSent)
        {
            verifyEmstatPico(line);                     // If version command is sent, calls the method to verify if connected device is EmStat Pico
        }
        else {
            if(processReceivedPackages(line)){          // Calls the method to process the received response packets
     ...
     */
    //TODO: Remove 'e' and change if else to switch
    private boolean processReceivedPackages(String readLine) {
        ...
        if (readLine.equals("\n")) {
            ...
            isComplete = true;
        } else if ('M' == readLine.charAt(0)) {
            txtResponse.append(String.format("%nReceiving measurement response...%n"));

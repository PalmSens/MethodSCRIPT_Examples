...
namespace EmStatConsoleExample
{
    class Program
    {
        ...
        static int NDataPointsReceived = 0;                                                        // The number of data points from received the measurement
        ...
        private static void ProcessReceivedPackets()
        {
            ...
            while (true)
            {
                ...
                if (readLine == "\n")
                    break;
                NDataPointsReceived++;                      // Increments the number of data points if the read line is not empty
        ...
        private static void ParsePackageLine(string responsePackageLine)
        {
            ...
            while (!(responsePackageLine.Substring(currentIndex) == "\n"))
            {
                ...
                Console.Write(MeasurementParameters[paramIdentifier] + "=" + string.Format("{0:0.000E+00}", paramValueWithPrefix).ToString() + "    ");
